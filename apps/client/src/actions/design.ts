...
const uploadImage = async (image: Uint8Array, title: string) => {
...
export const designgen = async (input: DesignGen): Promise<DesignGenRes> => {
    ...
    try {
        ...
        return { success: true, message: "Design created successfully", design: withUrl(design) };
    } catch (error) {
...
export const designupd = async (input: DesignUpd): Promise<DesignUpdRes> => {
    ...
    try {
        ...
        const design = await client.design.update({ where: { id: input.id }, data: { title: input.title, description: input.description, key, updatedAt: new Date() } });
        if (uploadedKey) await utapi.deleteFiles(existing.key).catch(() => undefined);
        return { success: true, message: "Design updated successfully", design: withUrl(design) };
    } catch (error) {
        if (uploadedKey) await utapi.deleteFiles(uploadedKey).catch(() => undefined);
        return { success: false, message: errorMessage(error) };
    }
