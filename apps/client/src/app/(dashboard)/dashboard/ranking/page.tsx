...
import { useAddKeyword, useDeleteKeyword, useGetKeywordRank, useRankList, useRefreshRank, useToggleRank } from '@/hooks/use-rank';
import { getKeywordRankApi } from '@/lib/client/rank';
...
const Ranking = () => {
  ...
  const { data: ranks, isPending } = useRankList();
  const addKeyword = useAddKeyword();
  const toggleRank = useToggleRank();
  const deleteRank = useDeleteKeyword();
  const refreshRank = useRefreshRank(); 

  const processedData = ranks?.keywords.sort((a, b) => {
    if (sortBy === "newest") {
  ...
  const handleAdd = async (e: React.SubmitEvent) => {
    ...
    try {
      const res = await addKeyword.mutateAsync({
        keyword: newKeyword,
        url: newUrl
      });
      ...
      // Poll for completion
      const id = res.tracking.id;
      const pollInterval = setInterval(async () => {
        try {
          const check = await getKeywordRankApi({ id })
          if (check.tracking.status !== "CHECKING") {
  ...
  const handleToggle = async (id: string) => {
    try {
      await toggleRank.mutateAsync({ id });
    } catch (err) {
  ...
  const handleRefresh = async (id: string) => {
    setRefreshing(id);
    try {
      await refreshRank.mutateAsync({ id })
        ...
        {isPending ? (
          ...
          <div className="space-y-3" style={{ animationDelay: "200ms" }}>
            {processedData?.map((kw) => {
              ...
              return (
                <div key={kw.id} className={`glass rounded-xl p-5 hover:bg-muted/50 transition-all ${!kw.active ? "opacity-50" : ""}`}>
                  <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <Link href={`/rank/${kw.id}`} className="text-base font-semibold text-foreground hover:text-primary transition-colors block truncate">
                        "{kw.keyword}"
                      </Link>
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <Link href={`/rank/${kw.id}`} className="p-2 rounded-lg hover:bg-muted text-muted-foreground hover:text-primary transition-all" title="View Details">
                        <ExternalLink size={16} />
                      </Link>
                      <button onClick={() => handleRefresh(kw.id)} disabled={refreshing === kw.id || kw.status === "CHECKING"} className="p-2 rounded-lg hover:bg-muted text-muted-foreground hover:text-primary transition-all disabled:opacity-30" title="Refresh Ranking">
                        <RefreshCw size={16} className={refreshing === kw.id ? "animate-spin" : ""} />
                      </button>
                      <button
                      ...
                      <button onClick={() => handleDelete(kw.id)} disabled={deleting === kw.id} className="p-2 rounded-lg hover:bg-danger/10 text-muted-foreground hover:text-danger transition-all disabled:opacity-50" title="Delete">
                        {deleting === kw.id ? <Loader2 size={16} className="animate-spin" /> : <Trash2 size={16} />}
                      </button>
                    </div>
                  </div>
                </div>
