...
import { useDeleteAnalyse, useListAnalyse } from "@/hooks/use-analyse";
import { Clock, Trash2, ExternalLink, Search, AlertCircle, Loader2, Filter, ArrowUpDown } from "lucide-react";
import Link from 'next/link';
import ScoreGauge from "@/components/modules/tracking/score-gauge";
import { useState } from "react";

const History = () => {
  const [page, setPage] = useState(1);
  const [limit, seLimit] = useState(12);
  const [deleting, setDeleting] = useState<string | null>(null);
  const [searchQuery, setSearchQuery] = useState("");
  const [statusFilter, setStatusFilter] = useState("all");
  const [sortBy, setSortBy] = useState("newest");

  const { data: analyses, isPending } = useListAnalyse({
    limit: `${limit}`,
    page: `${page}`
  });
  const deleteAnalysis = useDeleteAnalyse();

  const [totalPages, setTotalPages] = useState(analyses?.pagination.page || 1);

  const handleDelete = async (id: string) => {
    setDeleting(id);

    try {
      await deleteAnalysis.mutateAsync({
        id
      });
    } catch (err) {
  ...
  let processedData = analyses?.analyses;
    ...
    <div className="min-h-screen pt-16 md:pt-24 bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          ...
          <Link href="/analyze" className="bg-primary px-5 py-2.5 rounded-xl text-sm font-semibold text-primary-foreground hover:opacity-90 transition-opacity self-start" style={{ color: "var(--background)" }}>
            New Analysis
          </Link>
        </div>
        ...
        {isPending ? (
          ...
          <div className="space-y-3" style={{ animationDelay: "200ms" }}>
            {processedData?.map((a) => (
              <div key={a.id} className="glass rounded-xl p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4 hover:bg-muted/50 transition-all group">
                {/* Score */}
                ...
                <div className="flex-1 min-w-0">
                  <Link href={`/report/${a.id}`} className="text-sm font-medium text-foreground hover:text-primary transition-colors truncate block">
                    {(() => {
                      try {
                ...
                {a.status === "COMPLETED" && (
                  <div className="hidden lg:grid grid-cols-4 gap-4">
                    {[
                      { label: "SEO", value: a.seoScore },
                      { label: "Perf", value: a.performanceScore },
                      { label: "A11y", value: a.accessibilityScore },
                      { label: "BP", value: a.bestPracticesScore },
                    ].map((c) => (
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <Link href={`/report/${a.id}`} className="p-2 rounded-lg hover:bg-muted text-muted-foreground hover:text-primary transition-all" title="View Report">
                    <ExternalLink size={16} />
                  </Link>
                  <button onClick={() => handleDelete(a.id)} disabled={deleting === a.id} className="p-2 rounded-lg hover:bg-danger/10 text-muted-foreground hover:text-danger transition-all disabled:opacity-50" title="Delete">
